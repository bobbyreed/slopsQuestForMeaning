...
**Soft-locks.** It fingerprints the active scene plus every progression flag. If
that fingerprint doesn't move for `--stall` seconds (default 25) while keys are
being hammered, it reports it. This is the shape of the bug where losing to The
Render left the player in a room whose only exit was decorative.
